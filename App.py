...
st.set_page_config(
   page_title="Resume Data Parsing using NLP",
   page_icon='./Logo/DS logo.jpeg',
)
def run():
    st.title("Resume Data Parsing Using NLP")
    st.sidebar.markdown("# Choose User")
    activities = ["Normal User", "Admin"]
    choice = st.sidebar.selectbox("Choose among the given options:", activities)
    # st.sidebar.markdown(link, unsafe_allow_html=True)
    img = Image.open('./Logo/DS logo.jpeg')
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'sahim' and ad_password == '12345678':
                st.success("Welcome sahim")
                # Display Data
